...
export default function Level3({ onBack, onNextLevel, onLevelComplete, language = 'en' }) {
  ...
  }, [showIntroVideo, activatePower]); // Only showIntroVideo and activatePower needed
  ...
  return (
    <div className="game-container">
      ...
      <div className="ui-overlay">
        ...
        {showGameOverModal && (
          <div className="level3-super-loss-modal-wrapper">
            <div className="level3-super-loss-content-box glass-panel">
              <h2 className="level3-super-loss-title">
                {beersCollected / initialCollectibles.length >= 0.7 ? gameUI.goodTry : gameUI.youLost}
              </h2>
              <p className="level3-super-loss-subtitle">
              ...
              <div className="level3-super-loss-stats">
                <div className="level3-super-loss-stat-row">
                ...
                <div className="level3-super-loss-stat-row" style={{ color: '#FFD700', fontWeight: 'bold', marginTop: '10px' }}>
                  <span>{gameUI.totalScore}:</span>
                  <span>{finalScoreStats.total}</span>
                </div>
                <div className="level3-super-loss-stat-row" style={{ marginTop: '10px' }}>
              ...
              <div className="level3-super-buttons-row">
                {beersCollected / initialCollectibles.length >= 0.7 && onNextLevel ? (
                  <button className="level3-loss-btn-primary" style={{ background: 'linear-gradient(135deg, #4CAF50 0%, #2E7D32 100%)' }} onClick={onNextLevel}>
                    {gameUI.nextLevel}
                  </button>
                ) : (
                  <button className="level3-loss-btn-primary" onClick={restartLevel}>
                    {gameUI.tryAgain}
                  </button>
                )}

                <button className="level3-btn-secondary" onClick={onBack}>
                  {gameUI.backToMenu}
                </button>
              </div>
            </div>
        ...
        {showWinModal && (
          <div className="level3-super-victory-modal-wrapper">
            <div className="level3-super-victory-content-box glass-panel">
              <h2 className="level3-super-victory-title">{gameUI.victory}</h2>
              <p className="level3-super-victory-subtitle">{gameUI.levelCompleted}</p>

              <div style={{ margin: '20px 0' }}>
                <StarRating stars={3} />
              </div>

              <div className="level3-super-victory-stats">
                <div className="level3-super-victory-stat-row">
                  <span>{gameUI.baseScore}:</span>
                  <span>{finalScoreStats.score}</span>
                </div>
                <div className="level3-super-victory-stat-row">
                ...
                <div className="level3-super-victory-stat-total">
              ...
              <div className="level3-super-buttons-row">
                {onNextLevel && (
                  <button className="level3-btn-primary" onClick={onNextLevel}>
                    {gameUI.nextLevel}
                  </button>
                )}
                <button className="level3-btn-secondary" onClick={restartLevel}>
                  {gameUI.playAgain}
                </button>
                <button className="level3-btn-secondary" style={{ backgroundColor: '#333' }} onClick={onBack}>
                  {gameUI.backToMenu}
                </button>
              </div>
            </div>
