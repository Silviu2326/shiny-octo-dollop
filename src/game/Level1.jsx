...
export default function Level1({ onBack, onNextLevel, onLevelComplete, userId, language = 'en' }) {
  ...
  useEffect(() => {
    // Only clear if we really want to stop EVERYTHING on any touch up.
    // Given the new specific button logic, global clearing might interfere (e.g. if dragging off).
    // Let's rely on button leave/up logic mainly.
  }, []);
  ...
  return (
    <div className="game-container">
      ...
      <div className="ui-overlay">
        ...
        {showVictoryModal && (
          <div className="level1-super-victory-modal-wrapper">
            <div className="level1-super-victory-content-box glass-panel">
              <h2 className="level1-super-victory-title">{gameUI.victory}</h2>
              <p className="level1-super-victory-subtitle">{gameUI.levelCompleted}</p>

              <div style={{ margin: '20px 0' }}>
                <StarRating stars={3} />
              </div>

              <div className="level1-super-victory-stats">
                <div className="level1-super-victory-stat-row">
                ...
                <div className="level1-super-victory-stat-total">
                  {gameUI.totalScore}: {finalScoreStats.total}
                </div>
              </div>

              <div className="level1-super-buttons-row">
                {onNextLevel && (
                  <button className="level1-btn-primary" onClick={onNextLevel}>
                    {gameUI.nextLevel}
                  </button>
                )}
                <button className="level1-btn-secondary" onClick={restartLevel}>
                  {gameUI.playAgain}
                </button>
                <button className="level1-btn-secondary" style={{ backgroundColor: '#333' }} onClick={onBack}>
                  {gameUI.backToMenu}
                </button>
              </div>
            </div>
        ...
        {showGameOverModal && (
          <div className="level1-super-loss-modal-wrapper">
            <div className="level1-super-loss-content-box glass-panel">
              <h2 className="level1-super-loss-title">
                {beersCollected / initialCollectibles.length >= 0.7 ? gameUI.goodTry : gameUI.youLost}
              </h2>
              <p className="level1-super-loss-subtitle">
              ...
              <div className="level1-super-loss-stats">
                <div className="level1-super-loss-stat-row">
                  <span>{gameUI.baseScore}:</span>
                  <span>{finalScoreStats.score}</span>
                </div>
                <div className="level1-super-loss-stat-row">
                ...
                <div className="level1-super-loss-stat-row" style={{ color: '#FFD700', fontWeight: 'bold', marginTop: '10px' }}>
                  <span>{gameUI.totalScore}:</span>
                  <span>{finalScoreStats.total}</span>
                </div>
                <div className="level1-super-loss-stat-row" style={{ fontSize: '0.9em', marginTop: '15px' }}>
                  <span>{gameUI.beersCollected}:</span>
                  <span>{beersCollected}/{initialCollectibles.length}</span>
                </div>
                {beersCollected / initialCollectibles.length >= 0.7 && (
              ...
              <div className="level1-super-buttons-row">
                {beersCollected / initialCollectibles.length >= 0.7 && onNextLevel ? (
                  <button className="level1-loss-btn-primary" style={{ background: 'linear-gradient(135deg, #4CAF50 0%, #2E7D32 100%)' }} onClick={onNextLevel}>
                    {gameUI.nextLevel}
                  </button>
                ) : (
                  <button className="level1-loss-btn-primary" onClick={restartLevel}>
                    {gameUI.tryAgain}
                  </button>
                )}

                <button className="level1-btn-secondary" onClick={onBack}>
                  {gameUI.backToMenu}
                </button>
              </div>
            </div>
      ...
      {showIntroVideo && (
        <div className="intro-video-overlay" style={{
          ...
          <video
            ref={videoRef}
            src={language === 'es' ? "/assets/videos/nivel0 coolcat.mp4" : "/assets/videosingles/LEVEL 0 (1).mp4"}
            autoPlay
